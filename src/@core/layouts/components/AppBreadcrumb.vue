        ...
        <b-col cols="12">
          ...
          <div class="breadcrumb-wrapper">
            <b-breadcrumb>
              ...
              <b-breadcrumb-item
                v-for="item in getLocaleBreadCrumb"
                :key="item.text"
                :active="item.active"
                :to="item.to"
              >
                {{ $t(item.i18n) || item.text }}
              </b-breadcrumb-item>
            </b-breadcrumb>
          </div>
        </b-col>
...
export default {
  ...
  computed: {
    ...mapGetters('breadcrumb', ['GET_BREADCRUMB']),
    getLocaleBreadCrumb() {
      if (this.$route.meta.is_dynamic_route) return this.GET_BREADCRUMB
